...
function wrapper(plugin_info) {
    ...
    var changelog = [
        {
            version: '1.6.9',
            changes: [
                'NEW: Added a "Show Import Debug Info" toggle in settings (default off).',
                'UPD: Improved import history alert message with clearer debug information.',
            ],
        },
        {
            version: '1.6.8',
            changes: [
                'FIX: Restored Import History by handling portals without getDetails() and preventing one bad portal from aborting the whole import.',
            ],
        },
        {
            version: '1.6.7',
    ...
    self.showImportDebug = false; // Toggle for showing detailed import debug info

    // showNoHistoryStats:
    // Intel does not return history data for all portals.
    // When enabled, it shows the number of portals without history data.
    // This is intended for debugging or analyzing data coverage only,
    // and has no practical value for regular users.
    ...
    self.importFromOfficialHistory = function () {
        ...
        var errorCount = 0;
        var guids = Object.keys(window.portals || {});
        ...
        guids.forEach(function (guid) {
            ...
            var history = null;
            try {
                if (portal.options && portal.options.data && portal.options.data.history) {
                    history = portal.options.data.history;
                }

                if (!history && typeof portal.getDetails === 'function') {
                ...
                errorCount++;
                console.warn('[uniques-tools] failed to import history for portal', guid, e);
                return;
            }
            ...
            if (history.captured && !uniqueInfo.captured) {
                self.updateStatus(guid, 'captured', true);
                changed = true;
            } else if (history.visited && !uniqueInfo.visited) {
                self.updateStatus(guid, 'visited', true);
                changed = true;
            }

            if (history.scoutControlled && !uniqueInfo.scoutControlled) {
        ...
        var message = 'Imported official history for ' + count + ' portals.';
        if (self.showImportDebug) {
            if (noHistoryCount > 0) {
                message += '\n[Debug] ' + noHistoryCount + ' portals did not include history data from Intel.';
            }
            if (errorCount > 0) {
                message += '\n[Debug] Skipped ' + errorCount + ' portals due to read errors (see console).';
            }
        }
