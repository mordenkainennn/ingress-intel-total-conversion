...
// @name           IITC plugin: Uniques + Drone
// @category       Misc
// @version        0.2.8
// @description    Allow manual entry of portals visited/captured/drone-visited. Use the 'highlighter-uniques' plugin to show the uniques on the map. Supports sync.
// @id             uniques
// @namespace      https://github.com/IITC-CE/ingress-intel-total-conversion
// @updateURL      https://iitc.app/build/release/plugins/uniques.meta.js
// @downloadURL    https://iitc.app/build/release/plugins/uniques.user.js
...
function wrapper(plugin_info) {
    ...
    var changelog = [
        {
            version: '0.2.8',
            changes: ['Added Drone support: Manual checkbox, highlighting, and list column.'],
        },
        {
            version: '0.2.7',
    ...
    window.plugin.uniques.onPublicChatDataAvailable = function (data) {
        ...
        data.result.forEach(function (msg) {
            ...
            // Standard Visited/Captured logic (unchanged)
            if (
            ...
            // Note: Drone moves are rarely shown in public chat reliably enough to automate without extra plugins
        });
    };
    ...
    window.plugin.uniques.updateCheckedAndHighlight = function (guid) {
        ...
        if (guid === window.selectedPortal) {
            var uniqueInfo = window.plugin.uniques.uniques[guid],
                ...
                drone = (uniqueInfo && uniqueInfo.drone) || false; // NEW
            $('#visited').prop('checked', visited);
            $('#captured').prop('checked', captured);
            $('#drone').prop('checked', drone); // NEW
        }
    ...
    // NEW FUNCTION: Update Drone status
    window.plugin.uniques.updateDrone = function (drone, guid) {
        ...
        if (drone === uniqueInfo.drone) return;

        uniqueInfo.drone = drone;
        // Drone visits are independent of physical visits/captures, so we don't force visited=true
        ...
        highlight: function (data) {
            ...
            if (uniqueInfo) {
                if (uniqueInfo.captured) {
                    ...
                    // Not captured
                    if (uniqueInfo.drone) {
                        // Drone visited (distinct color)
                        style.fillColor = '#DC00FF'; // Magenta/Purple for Drone
                        style.fillOpacity = 0.6;
                    } else if (uniqueInfo.visited) {
                        style.fillColor = 'yellow';
                        style.fillOpacity = 0.6;
                    } else {
                        // No visit, No capture, No drone
                        style.fillColor = 'red';
...
').appendTo('head');
    ...
    window.plugin.uniques.setupContent = function () {
        ...
        window.plugin.uniques.contentHTML =
            ...
            '<label><input type="checkbox" id="drone" onclick="window.plugin.uniques.updateDrone($(this).prop(\'checked\'))"> Drone</label>' +
            '</div>';
    ...
    window.plugin.uniques.setupPortalsList = function () {
        window.addHook('pluginUniquesUpdateUniques', function (data) {
            ...
            if (!info) info = { visited: false, captured: false, drone: false };
            ...
            $(`[data-list-uniques="${data.guid}"].drone`).prop('checked', !!info.drone); // NEW
        });
        ...
        window.addHook('pluginUniquesRefreshAll', function () {
            $('[data-list-uniques]').each(function (i, element) {
                ...
                if (e.hasClass('drone')) e.prop('checked', !!info.drone); // NEW
            });
        });
        ...
        function uniqueValue(guid) {
            ...
            var val = 0;
            if (info.visited) val += 1;
            if (info.captured) val += 2;
            if (info.drone) val += 4; // Weight Drone separately
            return val;
        }
        ...
        window.plugin.portalslist.fields.push({
            ...
            format: function (cell, portal, guid) {
                ...
                createBox('visited', 'Portal visited?', !!info.visited, window.plugin.uniques.updateVisited);
                createBox('captured', 'Portal captured?', !!info.captured, window.plugin.uniques.updateCaptured);
                // Add Drone Checkbox to List
                createBox('drone', 'Drone visited?', !!info.drone, window.plugin.uniques.updateDrone);
            },
        });
    };
